...
export const TrackOrderModal: React.FC<TrackOrderModalProps> = ({ isOpen, onClose, initialOrderId = '' }) => {
  ...
  const handleTrack = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!orderId.trim()) {
      setError('Please enter your AEVY Order ID (e.g., AEVY-20260816-0001)');
      return;
    }
    ...
    try {
      ...
      setError(err.message || 'Unable to locate order. Please check the ID and try again.');
      setOrder(null);
    } finally {
      setLoading(false);
    }
          ...
          <div className="border-t border-[#2A2A2A] pt-6 space-y-6">
            <div className="flex flex-wrap items-center justify-between gap-2 bg-[#111111] p-4 border border-[#2A2A2A]">
              <div>
                ...
                <p className="font-serif text-lg font-medium text-[#F5F1E8]">{order.id}</p>
              </div>
            ...
            {order.status !== 'Cancelled' ? (
              ...
              <div className="p-3 bg-rose-950/40 text-rose-300 text-xs border border-rose-800/50">
                This order was cancelled. If you need assistance, please contact our concierge via WhatsApp.
              </div>
            )}
