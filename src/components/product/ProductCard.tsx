...
const minPrice =
  ...
  const primaryImage = product.image_url || (product.images && product.images[0]) || '/images/logo_n.jpg';
        ...
        <div>
          ...
          <div className="flex items-center justify-between text-xs text-[#6B6B6B] font-sans mb-3">
            ...
            <span className="text-[11px] text-[#8C8C8C] font-sans whitespace-nowrap">
              {variants.length > 0 ? variants.map((v) => v?.size).filter(Boolean).join(' / ') : 'Extrait'}
            </span>
          </div>
        </div>
