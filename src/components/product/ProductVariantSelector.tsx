...
const SIZE_LABELS: Record<string, string> = {
  ...
  '15ml': 'Travel Edition (15ml)',
  '30ml': 'Signature Flacon (30ml)',
  '50ml': 'Grand Edition (50ml)',
};
    ...
    <div className="space-y-3">
      <div className="flex items-baseline justify-between text-xs">
        ...
        <span className="text-[#6B6B6B]">
          {selectedVariant?.size ? (SIZE_LABELS[selectedVariant.size] || selectedVariant.size) : 'Choose Size'}
        </span>
      </div>
        ...
        {variants.filter(Boolean).map((variant) => {
          ...
          return (
            <button
              key={variant.id}
              type="button"
              id={`size-select-${variant.size || variant.id}`}
