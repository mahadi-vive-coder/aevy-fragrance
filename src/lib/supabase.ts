 ...
 */
export function mapSupabaseProduct(row: any, productImages: any[] = []): Product {
  ...
  .sort((a, b) => {
    // Primary image first, then sort_order
...
export async function getSupabaseProducts(forAdmin: boolean = false): Promise<Product[]> {
  try {
    ...
    if (imagesError) {
      console.warn('AEVY product_images query warning:', imagesError.message);
    }
    ...
    // Map products with their complete image gallery
    return products.map(product => {
      const images = imagesByProduct.get(String(product.id)) || [];

      return mapSupabaseProduct(product, images);
    });
...
export async function getSupabaseProductBySlug(slug: string): Promise<Product | null> {
  try {
    ...
    if (imagesError) {
      console.warn('AEVY product detail images warning:', imagesError.message);
    }

    return mapSupabaseProduct(data, productImages || []);
